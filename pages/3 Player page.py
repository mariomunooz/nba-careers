...
def hex_plot2(proportions1, proportions2, labels, title1, title2):
...
if filtered_players:
    ...
    labels = ["ppg","astpg","rebpg","stpg","blpg","efgpg"]
    ...
    st.write('### Career Averages vs Peak Performance')
    hex_plot2(proportions, proportions_peak,labels, 'Player Career Averages', 'Player Peak Stats Per Game')
    ...
    if st.button("Show season stats:"):
        # Ordered array of class names
        class_mapping = {0: 'Out of the League', 1:'Roster', 2:'Rotation', 3:'Starter', 4:'All-Star', 5:'Elite'}
        # Map the 'class' column to class names
        player_data['season_outcome'] = player_data['season_outcome'].map(class_mapping)
        player_data['nbapersonid']    = player_data['nbapersonid'].astype(str)
        player_data['draftyear']      = player_data['draftyear'].astype(str)
        player_data['season']         = player_data['season'].astype(str)
        st.dataframe(player_data, hide_index= True)

else: 
    st.write("No matching player found. Please try a different name.")
